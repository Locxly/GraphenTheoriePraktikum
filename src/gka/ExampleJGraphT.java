 ...
 * Dokumentationskopf ------------------
 * 
 * Team 6: Stephan Hšlscher, Milena Dreier
 * 
...
public class ExampleJGraphT {
	 ...
	 * 
	 * @param args
	 *            in the first arg the file name could be inserted. If empty
	 *            take the default one.
	 * @throws IOException
	 *             Error during process the input file.
	 */
	public static void main(String[] args) throws IOException {
		...
		if (createdGraph != null) {
			...
			// try {
			// depthFirstSearchForVertex(createdGraph, startVertex,
			// destinationVertex);
			// } catch (Exception e) {
	 ...
	 * 
	 * @param args
	 *            the argument array from start up
	...
	private static void startUpProcedure(String[] args)
		...
		if (args != null && args.length != 0 && args[0].equals("howto")) {
			System.out
					.println("Please note the following use instruction : Type arg[0] as ...");
			System.out.println("0 -> default use of params.");
			System.out.println("1 -> arg[1] contains filename.");
			System.out
					.println("2 -> arg[1] start vertex, arg[2] destination vertex.");
			System.out
					.println("3 -> arg[1] filename, arg[2] start vertex, arg[3] destination vertex.");
			System.exit(0);
		}
	...
	/**
	 * Search the way between startVertex and destination vertex using the
	...
	private static void recursiveBreadthFirstStep(
		...
		// If the case happens we could skip the processing of this vertex.
		if (neighborList == null
				|| neighborList.size() == 0
				|| (neighborList.size() == 1 && neighborList
						.contains(predecessorVertex))) {
			System.out
					.println("Vertex ["
							+ workingVertex
							+ "] has no neighbours exept the pedecessor. It's time for the recursive step.");
			return;
		}
		...
		for (String vertex : neighborList) {
			...
			displayFinalWayToVertex();
			// If the actual vertex is the destination vertex we can finish the
			// search.
			if (vertex.equals(destinationVertex)) {
				...
				// If the actual vertex is not the destination vertex we will
				// remove it from the way to destination.
		...
		for (String vertex : neighborList) {
			...
			if (vertex.equals(predecessorVertex)) {
				System.out.println("Skip vertex [" + vertex
						+ "]. It is the predecessor [" + workingVertex + "]");
			} else {
				// If the actual vertex is already contained in the way to
				// destination list we will have a circle. So we skip the
				// processing of this vertex.
				if (vertexWayList.contains(vertex)) {
					break;
				}
				...
				recursiveBreadthFirstStep(createdGraph, vertex,
						destinationVertex, workingVertex);
				// If we found the destination vertex during the recursive
				// process we can skip more processing.
				if (breadthFirstFinish) {
					return;
				} else {
					// If we didn't find the destination vertex we can remove
					// this vertex from way to destination.
					removeLastVertexFromWayToVertex(vertex);
				}
			}
		}
	...
	/**
	 * This method removes the last element of the the given vertex from the
	 * vertex way to destination list.
	 * 
	 * @param vertex
	...
	private static void recursiveDepthFirstStep(
		...
		// in mind that this could maybe never happen because the predecessor is
		// always contained in the neighbor list.
		...
		for (String vertex : neighborList) {
			...
			// If the actual vertex is the predecessor vertex we skip this
			// vertex.
			if (vertex.equals(predecessorVertex)) {
				;
			} else {
				...
				// If the actual vertex is the destination vertex we have
				// finished the search and can return to primary recursive
				// start.
				if (vertex.equals(destinationVertex)) {
					System.out.println("Vertex found.");
					// Set the depth first finish mark to 'TRUE'. This shows
				...
				// The next recursive step for the actual vertex. We go deeper
				// into the graph on this vertex.
				recursiveDepthFirstStep(createdGraph, vertex,
						destinationVertex, workingVertex);

				// If the search is not finished for this vertex we will remove
				// it from way to destination list because the vertex is not
				// part of the way to destination.
				if (!depthFirstFinish) {
					removeLastVertexFromWayToVertex(vertex);
				}
			}
